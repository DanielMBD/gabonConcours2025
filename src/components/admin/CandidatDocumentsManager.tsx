        ...
        try {
          await emailService.sendDocumentValidationNotification({
            ...
            document: {
              ...
              nom: selectedDocument.nom_document
            },
            statut: validationStatus,
            commentaire: validationComment
          });
              ...
              {documents.map((document) => (
                <div key={document.id} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h4 className="font-medium">{document.nom_document}</h4>
                      <p className="text-sm text-muted-foreground">
                        {document.type_document} • {(document.taille_fichier / 1024 / 1024).toFixed(2)} MB
                      </p>
          ...
          <div className="space-y-4">
            {selectedDocument && (
              <div className="p-4 bg-muted rounded-lg">
                <p className="font-medium">{selectedDocument.nom_document}</p>
                <p className="text-sm text-muted-foreground">{selectedDocument.type_document}</p>
              </div>
            )}
            <div>
              <Label htmlFor="status">Statut de validation</Label>
              <Select value={validationStatus} onValueChange={(value) => setValidationStatus(value as 'valide' | 'rejete')}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              <Label htmlFor="comment">Commentaire {validationStatus === 'rejete' ? '(obligatoire)' : '(optionnel)'}</Label>
              <Textarea
