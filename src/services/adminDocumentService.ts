...
class AdminDocumentService {
  ...
  // Valider un document
  async validateDocument(documentId: number, validation: DocumentValidationRequest): Promise<{ success: boolean; message?: string }> {
    try {
      console.log('AdminDocumentService: Validation document:', documentId, validation);
      const response = await apiService.updateDocumentStatus(documentId.toString(), validation.statut, validation.commentaire);
      return {
        success: response.success || false,
        message: response.message
      };
  ...
  async getDocumentStats(): Promise<{ success: boolean; data: DocumentStats; message?: string }> {
    try {
      const response = await apiService.makeRequest<{ stats: any[]; totals: DocumentStats }>('/document-validation/stats', 'GET');
