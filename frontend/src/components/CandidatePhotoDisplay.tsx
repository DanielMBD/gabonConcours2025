...
const CandidatePhotoDisplay: React.FC<CandidatePhotoDisplayProps> = ({
  ...
  return (
    <div className={`${sizeClasses[size]} rounded-full overflow-hidden border-4 border-primary/20 bg-muted flex items-center justify-center ${className}`}>
      {photoUrl ? (
        <img
          src={photoUrl}
      ...
      <div className={`photo-fallback w-full h-full bg-muted flex items-center justify-center ${photoUrl ? 'hidden' : 'flex'}`}>
