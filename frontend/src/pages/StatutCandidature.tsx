...
import { CheckCircle, Clock, FileText, CreditCard, User, AlertCircle, Download, GraduationCap, BookOpen, Trophy } from 'lucide-react';
...
const StatutCandidature = () => {
  ...
  const getStatutBadge = (statut: string) => {
    switch (statut) {
      case 'termine': return <Badge className="bg-green-500 text-white">Terminé</Badge>;
      case 'en-cours': return <Badge className="bg-blue-500 text-white">En cours</Badge>;
  ...
  const getStatutGlobal = () => {
    switch (progression.etapeActuelle) {
      case 'complete': return { label: 'Candidature complète', color: 'text-green-600', bg: 'bg-green-50' };
      case 'paiement': return { label: 'Prêt pour le paiement', color: 'text-blue-600', bg: 'bg-blue-50' };
      case 'documents': return { label: 'Upload de documents', color: 'text-orange-600', bg: 'bg-orange-50' };
      default: return { label: 'Inscription en cours', color: 'text-gray-600', bg: 'bg-gray-50' };
    }
  };
              ...
              <div className="space-y-3">
                {documents.map((doc: any) => (
                  <div key={doc.id} className="flex items-center justify-between p-3 border rounded-lg">
                    ...
                    <div className="flex items-center space-x-2">
                      <Badge variant={doc.statut === 'valide' ? 'default' : doc.statut === 'rejete' ? 'destructive' : 'secondary'}>
                        {doc.statut}
                      </Badge>
                      <Button
                        variant="outline"
