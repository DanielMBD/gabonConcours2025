...
// POST /api/dossiers - Créer un nouveau dossier avec plusieurs documents
router.post('/', (req, res) => {
  uploadMultiple(req, res, async (err) => {
    try {
      ...
      console.log('Requête reçue:', req.body, req.files);
...
router.put('/:id', upload.single('docdsr'), async (req, res) => {
  try {
    ...
    const dossier = await Document.updateStatus(id, 'en_attente'); // Assuming update logic is in updateStatus
